...
Use your actual accessible Git remote and an existing revision for a Git source. A local archive or configured remote does not mean a public release exists. The portable Agent Plugins archive contains `plugin.json`, `mcp.json`, three skills and the bundled runtime. Resolve paths from its extracted location; no developer-machine path is required.
